...
int led = 13; // test led
//unsig{:P.
int start_time = 120; // value neccesary to enter maze solving logic
int startFlag = 0;
...
StartTick();// if we given signal(if center sensor reading reads more than 3s, than startflag = 1;
mazesolve();// will only be entered if startflag = 1;
}
...
//----SM to search for signal to enter into agortim----------------------------------------- 
int stateSM[] = {0, 1};
int state = stateSM[0];
...
//function to troubleshoot sensor_readings--------------------------------
void output_sensor_reading(){ 
  Serial.print("Right: ");
...
//enum dicision_states{dicision, go_straight, turn_left turn_right, reverse };
int Maze_SM[] = {0, 1, 2, 3, 4};
int Dicision_state = Maze_SM[0];

void mazesolve(){
  
  if ( startFlag == 1){
    switch(Dicision_state){ //transistions---------------------------
      // one turning scenarios---------------------------------------------------
      case 0:
        if ( (left > 200) && (right > 200) && ( center < 200)){ // when wall on left and right
         Dicision_state = Maze_SM[1]; // state = go_straight;
        }

        else if ( (left > 200) && (right < 200) && ( center > 200)){ // walls on the left and in front
         Dicision_state = Maze_SM[3];// state = go_rigjt
        }

        
        else if ( (left < 200) && (right > 200) && ( center > 200)){ // walls on the right and in front
         Dicision_state = Maze_SM[2];// state = left
        }

        else if((left > 200) && (right > 200) && ( center > 200)){ // walls on all sides, mouse needs to reverse
           Dicision_state = Maze_SM[4];// state = reverse
        }
       // ---------------two possible turn scenarios, where random takes place---------------
       else  if ( (left > 200) && (right < 200) && ( center < 200)){ // when wall on left 
          if(millis()% 2 == 1){
           Dicision_state = Maze_SM[3];// state = turn_right; 
          }
          else{
         Dicision_state = Maze_SM[1]; //state = go_straight;
          }
           ...
           if(millis()% 2 == 1){
            Dicision_state = Maze_SM[2]; //state = turn_left;
            //
          }
          else{
             Dicision_state = Maze_SM[1];//state = go_straight;
          ...
          if(millis() % 2 == 0){
           Dicision_state = Maze_SM[2] ; //dicision_state = turn_left;
           }
          else{
           Dicision_state = Maze_SM[3];  //dicision_state_turn_right;
          }
        }
      break;

      case 1: 
       Dicision_state = Maze_SM[0];//dicision_ state = dicision;
      break;
      case 2: 
       Dicision_state = Maze_SM[0];//dicision_state = dicision;
      break;
      case 3:
       Dicision_state = Maze_SM[0];// dicision_state = dicision;
      break;
      case 4:
      Dicision_state = Maze_SM[0];
    }//transistions
    switch(Dicision_state){// actions---------------
      ...
      break;
      case 1:// go straiht
      //call straight function;
      after_turn();
      break;
      case 2:// go left
       //call left truning function
       after_turn();
      break;
      case 3:// go right 
      //call right function;
...
//delays and after turn functions---------------------------
void after_turn(){
  unsigned int pid_delay_time = 2500;
  unsigned int cnt;
